...
pub fn contracttype(metadata: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let derived = match &input.data {
        Data::Struct(s) => match s.fields {
            Fields::Named(_) => {
                derive_type_struct(&args.crate_path, vis, ident, s, gen_spec, &args.lib)
            }
            Fields::Unnamed(_) => {
                derive_type_struct_tuple(&args.crate_path, vis, ident, s, gen_spec, &args.lib)
            }
        ...
        Data::Enum(e) => {
            ...
            if count_of_int_variants == 0 {
                ...
                derive_type_enum_int(&args.crate_path, vis, ident, e, gen_spec, &args.lib)
            } else {
